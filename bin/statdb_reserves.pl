...
# Récupération des dates de prêt
sub ReserveIssueDate {
	my $dbh = GetDbh() ;
	my $req = "SELECT reserve_id, itemnumber, borrowernumber, waitingdate FROM statdb.stat_reserves WHERE found = 'F' AND waitingdate IS NOT NULL AND issuedate IS NULL AND itemnumber IS NOT NULL" ;
	my $sth = $dbh->prepare($req);
	$sth->execute();
...
WHERE itemnumber = ?
AND borrowernumber = ?
AND issuedate BETWEEN ? AND ? + INTERVAL 10 DAY
SQL
	my $sth = $dbh->prepare($req) ;

...
sub reserves_new {
    ...
    foreach my $table (@tables) {
        $req = <<SQL ;
INSERT INTO statdb.stat_reserves2 (reserve_id, borrowernumber, reservedate, biblionumber, branchcode, notificationdate, cancellationdate, priority, found, timestamp, itemnumber, waitingdate, expirationdate, age, sexe, ville, iris, branchcode_borrower, categorycode, fidelite, mobile, courriel)
SELECT
    r.reserve_id,
    r.borrowernumber,
    r.reservedate,
...
sub reserves_maj {
    ...
    foreach my $table (@tables) {
        ...
        $req = <<SQL ;
UPDATE statdb.stat_reserves2 s 
...
sub new_reserve_id {
    ...
    my $dbh = GetDbh() ;
    my $req = "SELECT reserve_id FROM statdb.stat_reserves2 WHERE reservedate = ? " ;
...
sub reserves_infos {
    ...
    my $dbh = GetDbh() ;
    my $req = "SELECT branchcode, biblionumber FROM statdb.stat_reserves2 WHERE reserve_id = ? " ;
    my $sth = $dbh->prepare($req);
    $sth->execute($reserve_id);
